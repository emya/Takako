export const updateTrip = (index, destination) => {
  return (dispatch, getState) => {
    ...
    let body = JSON.stringify({destination, });
    let tripId = getState().trips[index].id;

    return fetch(`/api/trips/${tripId}/`, {headers, method: "PUT", body})
      .then(res => {
        if (res.status < 500) {
      ...
      .then(res => {
        if (res.status === 200) {
          return dispatch({type: 'UPDATE_TRIP', TRIP: res.data, index});
...
export const deleteTrip = index => {
  return (dispatch, getState) => {
    ...
    return fetch(`/api/trips/${tripId}/`, {headers, method: "DELETE"})
      .then(res => {
        if (res.status === 204) {
          return {status: res.status, data: {}};
      ...
      .then(res => {
        if (res.status === 204) {
          return dispatch({type: 'DELETE_TRIP', index});
        } else if (res.status === 401 || res.status === 403) {
          dispatch({type: "AUTHENTICATION_ERROR", data: res.data});
          throw res.data;
        }
...
export const fetchTrip = (tripId) => {
  return (dispatch, getState) => {
    ...
    return fetch(`/api/trips/${tripId}/`, {headers, })
      ...
      .then(res => {
        if (res.status === 200) {
          return dispatch({type: 'FETCH_TRIP', trip: res.data});
        } else if (res.status === 401 || res.status === 403) {
          dispatch({type: "AUTHENTICATION_ERROR", data: res.data});
          throw res.data;
        }
...
export const fetchTrips = (userId) => {
  return (dispatch, getState) => {
    ...
    return fetch(`/api/trips/?userId=${userId}`, {headers, })
      ...
      .then(res => {
        if (res.status === 200) {
          return dispatch({type: 'FETCH_TRIPS', trips: res.data});
        } else if (res.status === 401 || res.status === 403) {
          dispatch({type: "AUTHENTICATION_ERROR", data: res.data});
          throw res.data;
        }
